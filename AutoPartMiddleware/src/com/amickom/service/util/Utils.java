...
public class Utils {
    ...
    public static void fileMove(String sourceFile, String destinationFile) {
        ...
        try {
            ...
            outFile = new File(destinationFile + inFile.getName()
                    + Utils.HORA.format(Calendar.getInstance().getTime()));
    ...
    public static void formateaMonto(BigDecimal monto, StringBuffer detalle) {
        ...
        if (monto.signum() == -1) {
            detalle.append("-");
            detalle.append(Utils.rellenaEspacios(monto.movePointRight(2).toString(), "0", 15, 1)); // importe
        } else {
            //detalle.append("+");
            detalle.append(Utils.rellenaEspacios(monto.movePointRight(2).toString(), "0", 16, 1)); // importe
        }
    }

    public static synchronized Properties cargaPropiedadesClass(String archivo)
            throws RefaccionariaException {
        Properties props = new Properties();
        try {
            props.clear();
            props.load(Utils.class.getClassLoader().
                    getResourceAsStream(archivo));
        } catch (FileNotFoundException e) {
            e.printStackTrace();
            throw new RefaccionariaException(
                    e.getMessage(), e);
        } catch (IOException e) {
            e.printStackTrace();
            throw new RefaccionariaException(
